...
public final class MqttTopicConstants {
    ...
    static {
        TOPIC_PREFIXES = Arrays.stream(MqttTopicConstants.class.getDeclaredFields())
                .filter(field -> Modifier.isPublic(field.getModifiers()) &&
                        Modifier.isStatic(field.getModifiers()) &&
                        field.getType().equals(String.class) &&
                        field.getName().endsWith("_TOPIC"))
                .map(field -> {
                    try {
                        return (String) field.get(null);
                    } catch (IllegalAccessException e) {
                        // 로깅 또는 예외 처리
                        throw new RuntimeException("Failed to access topic constant field", e);
                    }
                })
                .filter(value -> value != null && value.endsWith(ALL_SUB_LEVELS))
                .map(value -> value.substring(0, value.length() - ALL_SUB_LEVELS.length()))
                .distinct()
